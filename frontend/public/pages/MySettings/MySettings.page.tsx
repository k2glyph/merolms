...
import { Modal, Form, Button, PageTitle, Input, Select, SelectOption, ImageUploader, Header } from "@meroedu/components"

import { UserSettings, UserAvatarType, ImageUpload } from "@meroedu/models"
import { Failure, actions, Meroedu } from "@meroedu/services"
...
export default class MySettingsPage extends React.Component<MySettingsPageProps, MySettingsPageState> {
  constructor(props: MySettingsPageProps) {
    ...
    this.state = {
      showModal: false,
      changingEmail: false,
      avatarType: Meroedu.session.user.avatarType,
      newEmail: "",
      name: Meroedu.session.user.name,
          ...
          <div className="w-max-7xl">
            <Form error={this.state.error}>
              <Input
                ...
                field="email"
                value={this.state.changingEmail ? this.state.newEmail : Meroedu.session.user.email}
                maxLength={200}
              ...
              >
                <p className="text-muted">
                  {Meroedu.session.user.email || this.state.changingEmail ? (
              ...
              >
                ...
                {this.state.avatarType === UserAvatarType.Custom && (
                  <ImageUploader field="avatar" onChange={this.setAvatar} bkey={Meroedu.session.user.avatarBlobKey}>
                    <p className="text-muted">
            ...
            <div className="mt-8">{Meroedu.session.user.isCollaborator && <APIKeyForm />}</div>
            <div className="mt-8">
              <DangerZone />
            </div>
          </div>
