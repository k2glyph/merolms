...
func TestBus_SimpleMessage(t *testing.T) {
	...
	cmd := &SayHelloCommand{Name: "Meroedu"}
	err := bus.Dispatch(ctx, cmd)
	Expect(err).IsNil()
	Expect(cmd.Result).Equals("Good Morning Meroedu")
}
...
func TestBus_MessageIsNotPointer_ShouldPanic(t *testing.T) {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			Expect(panicText).Equals("'github.com/k2glyph/meroedu/app/pkg/bus_test.SayHelloCommand' is not a pointer")
		}
	}()
...
func TestBus_OverwriteService(t *testing.T) {
	...
	err := bus.Dispatch(context.Background(), cmd)
	Expect(err).IsNil()
	Expect(cmd.Result).Equals("Hello Meroedu")
}
...
func TestBus_MultipleListeners(t *testing.T) {
	...
	bus.Publish(context.Background(), &SayHelloCommand{Name: "Meroedu"})
	Expect(value1).Equals("Meroedu")
	Expect(value2).Equals("Meroedu")
}
