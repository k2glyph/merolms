...
func TestIsCustomDomain(t *testing.T) {
	...
	Expect(req.URL.String()).Equals("https://demo.test.meroedu.io/api/hello?value=Jon")
...
func TestRequest_FullURL(t *testing.T) {
	...
	req1 := web.WrapRequest(&http.Request{Host: "demo.test.meroedu.io:3000"})
	Expect(req1.IsCustomDomain()).IsFalse()

	req2 := web.WrapRequest(&http.Request{Host: "demo.test.meroedu.io"})
