...
func TestRenderer_Home_SSR(t *testing.T) {
	...
	buf := new(bytes.Buffer)
	ctx := newGetContext("https://demo.test.meroedu.io:3000/", map[string]string{
...
func TestRenderer_AuthenticatedUser(t *testing.T) {
	...
	ctx.SetUser(&entity.User{
		...
		AvatarURL:  "https://demo.test.meroedu.io:3000/static/avatars/gravatar/5/Jon%20Snow",
	})
	renderer := web.NewRenderer()
	renderer.Render(buf, http.StatusOK, web.Props{
		Title:       "My Page Title",
		...
		q.Result = []*dto.OAuthProviderOption{
			{
				...
				DisplayName:      "Google",
				ClientID:         "1234",
				URL:              "https://demo.test.meroedu.io:3000/oauth/google",
				CallbackURL:      "https://demo.test.meroedu.io:3000/oauth/google/callback",
