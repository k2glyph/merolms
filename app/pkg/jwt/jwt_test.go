...
func TestJWT_Decode(t *testing.T) {
	...
	decoded, err := jwt.DecodeMeroeduClaims(token)
	Expect(err).IsNil()
...
func TestJWT_Decode_DifferentSignMethod(t *testing.T) {
	...
	jwtToken := jwtgo.NewWithClaims(jwtgo.GetSigningMethod("none"), &jwt.MeroeduClaims{
		UserID:    424,
		UserName:  "Jon Snow",
		UserEmail: "jon.snow@got.com",
	})
...
func TestJWT_DecodeChangedToken(t *testing.T) {
	...
	decoded, err := jwt.DecodeMeroeduClaims(token + "foo")
	Expect(err).IsNotNil()
	Expect(decoded).IsNil()
}
