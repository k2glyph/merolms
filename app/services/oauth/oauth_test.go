...
func TestGetAuthURL_Facebook(t *testing.T) {
	...
	ctx := newGetContext("http://login.test.meroedu.io:3000")
	...
	Expect(err).IsNil()
	Expect(authURL.Result).Equals("https://www.facebook.com/v3.2/dialog/oauth?client_id=FB_CL_ID&redirect_uri=http%3A%2F%2Flogin.test.meroedu.io%3A3000%2Foauth%2Ffacebook%2Fcallback&response_type=code&scope=public_profile+email&state=http%3A%2F%2Fexample.org%7C456")
}
...
func TestGetAuthURL_Google(t *testing.T) {
	...
	err := bus.Dispatch(ctx, authURL)
	Expect(err).IsNil()
	Expect(authURL.Result).Equals("https://accounts.google.com/o/oauth2/v2/auth?client_id=GO_CL_ID&redirect_uri=http%3A%2F%2Flogin.test.meroedu.io%3A3000%2Foauth%2Fgoogle%2Fcallback&response_type=code&scope=profile+email&state=http%3A%2F%2Fexample.org%7C123")
}
...
func TestGetAuthURL_GitHub(t *testing.T) {
	...
	err := bus.Dispatch(ctx, authURL)
	Expect(err).IsNil()
	Expect(authURL.Result).Equals("https://github.com/login/oauth/authorize?client_id=GH_CL_ID&redirect_uri=http%3A%2F%2Flogin.test.meroedu.io%3A3000%2Foauth%2Fgithub%2Fcallback&response_type=code&scope=user%3Aemail&state=http%3A%2F%2Fexample.org%7C456")
}
...
func TestGetAuthURL_Custom(t *testing.T) {
	...
	err := bus.Dispatch(ctx, authURL)
	Expect(err).IsNil()
	Expect(authURL.Result).Equals("https://example.org/oauth/authorize?client_id=CU_CL_ID&redirect_uri=http%3A%2F%2Flogin.test.meroedu.io%3A3000%2Foauth%2F_custom%2Fcallback&response_type=code&scope=profile+email&state=http%3A%2F%2Fexample.org%7C456")
}
