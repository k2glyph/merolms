...
var testCases = []struct {
...
}{
	{
		"Avengers",
		[]string{
			"http://avengers.test.meroedu.io",
			"http://avengers.test.meroedu.io:3000",
		},
	},
	{
		"Demonstration",
		[]string{
			"http://demo.test.meroedu.io",
			"http://demo.test.meroedu.io:1231",
			"http://demo.test.meroedu.io:80",
		},
	},
}
...
func TestMultiTenant(t *testing.T) {
	...
	bus.AddHandler(func(ctx context.Context, q *query.GetTenantByDomain) error {
		if q.Domain == "avengers.test.meroedu.io" {
			q.Result = mock.AvengersTenant
			return nil
		} else if q.Domain == "demo.test.meroedu.io" {
...
func TestMultiTenant_SubSubDomain(t *testing.T) {
	...
	bus.AddHandler(func(ctx context.Context, q *query.GetTenantByDomain) error {
		if q.Domain == "demo.test.meroedu.io" {
			q.Result = mock.DemoTenant
			return nil
		}
		return app.ErrNotFound
	...
	status, _ := server.WithURL("http://demo.demo.test.meroedu.io").Execute(func(c *web.Context) error {
		if c.Tenant() == nil {
			return c.Ok(web.Map{})
		}
...
func TestMultiTenant_DisabledTenant_ShouldNotSetInContext(t *testing.T) {
	...
	status, _ := server.WithURL("http://avengers.test.meroedu.io").Execute(func(c *web.Context) error {
		return c.Ok(nil)
	})
...
func TestMultiTenant_CanonicalHeader(t *testing.T) {
	...
	var testCases = []struct {
		...
		{
			"http://avengers.test.meroedu.io/posts",
			"http://feedback.theavengers.com/posts",
			false,
		},
		{
			"http://avengers.test.meroedu.io/posts?q=1",
...
func TestRequireTenant_MultiHostMode_ValidTenant(t *testing.T) {
	...
	status, response := server.WithURL("http://avengers.test.meroedu.io").Execute(func(c *web.Context) error {
		return c.String(http.StatusOK, c.Tenant().Name)
	})
...
func TestBlockLockedTenants_ActiveTenant(t *testing.T) {
	...
	status, response := server.
		WithURL("http://demo.test.meroedu.io").
		OnTenant(mock.DemoTenant).
		Execute(func(c *web.Context) error {
			return c.String(http.StatusOK, c.Tenant().Name)
		})
...
func TestBlockLockedTenants_LockedTenant(t *testing.T) {
	...
	status, _ := server.
		WithURL("http://demo.test.meroedu.io/api/v1/posts").
		OnTenant(mock.DemoTenant).
		Execute(func(c *web.Context) error {
			return c.String(http.StatusOK, c.Tenant().Name)
		})
