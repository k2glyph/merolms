...
func TestManageBillingHandler_RedirectWhenUsingCNAME(t *testing.T) {
	...
	Expect(response.Header().Get("Location")).Equals("https://demo.test.meroedu.io/admin/billing")
}
...
func TestManageBillingHandler_ReturnsCorrectBillingInformation(t *testing.T) {
	...
	code, page := server.
		OnTenant(mock.DemoTenant).
		AsUser(mock.JonSnow).
		WithURL("https://demo.test.meroedu.io/admin/billing").
...
func TestGenerateCheckoutLinkHandler(t *testing.T) {
	...
	server := mock.NewServer()
	code, json := server.
		WithURL("http://demo.test.meroedu.io/_api/billing/checkout-link").
		OnTenant(mock.DemoTenant).
		AsUser(mock.JonSnow).
