...
func TestSignUpHandler_MultiTenant(t *testing.T) {
	...
	code, _ := server.
		WithURL("http://login.test.meroedu.io/signup").
...
func TestSignUpHandler_MultiTenant_WrongURL(t *testing.T) {
	...
	code, response := server.
		WithURL("http://demo.test.meroedu.io/signup").
		Execute(handlers.SignUp())

	Expect(code).Equals(http.StatusTemporaryRedirect)
	Expect(response.Header().Get("Location")).Equals("http://login.test.meroedu.io/signup")
}
...
func TestCreateTenantHandler_WithSocialAccount(t *testing.T) {
	...
	ExpectMeroeduToken(cookie.Value, newUser)
