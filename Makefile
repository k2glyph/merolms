...
LDFLAGS += -X github.com/k2glyph/meroedu/app/pkg/env.commithash=${COMMITHASH}
...
run: ## Run Meroedu
	godotenv -f .env ./meroedu

migrate: ## Run all database migrations
	godotenv -f .env ./meroedu migrate
...
test-server: build-server build-ssr ## Run all server tests
	godotenv -f .test.env ./meroedu migrate
