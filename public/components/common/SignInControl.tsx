...
import { SocialSignInButton, Form, Button, Input, Message } from "@meroedu/components"
import { Divider } from "@meroedu/components/layout"
import { device, actions, Failure, isCookieEnabled } from "@meroedu/services"
...
export const SignInControl: React.FunctionComponent<SignInControlProps> = (props) => {
  ...
  const [showEmailForm, setShowEmailForm] = useState(meroedu.session.tenant ? meroedu.session.tenant.isEmailAuthAllowed : true)
  ...
  const providersLen = meroedu.settings.oauth.length
      ...
      {props.useEmail &&
        (showEmailForm ? (
          <div>
            ...
            {!meroedu.session.tenant.isEmailAuthAllowed && (
