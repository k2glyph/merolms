...
import { Comment, Post, ImageUpload } from "@meroedu/models"
import { Avatar, UserName, Moment, Form, TextArea, Button, Markdown, Modal, ImageViewer, MultiImageUploader, Dropdown, Icon } from "@meroedu/components"
import { HStack } from "@meroedu/components/layout"
import { formatDate, Failure, actions } from "@meroedu/services"
import { useMeroedu } from "@meroedu/hooks"
import IconDotsHorizontal from "@meroedu/assets/images/heroicons-dots-horizontal.svg"
...
export const ShowComment = (props: ShowCommentProps) => {
  ...
  const canEditComment = (): boolean => {
    if (meroedu.session.isAuthenticated) {
      return meroedu.session.user.isCollaborator || props.comment.user.id === meroedu.session.user.id
    }
    return false
  }
  ...
  const editedMetadata = !!comment.editedAt && !!comment.editedBy && (
    <span data-tooltip={`This comment has been edited by ${comment.editedBy.name} on ${formatDate(meroedu.currentLocale, comment.editedAt)}`}>· edited</span>
  )
          ...
          <HStack justify="between">
            <HStack>
              ...
              <div className="text-xs">
                · <Moment locale={meroedu.currentLocale} date={comment.createdAt} /> {editedMetadata}
              </div>
            </HStack>
