...
import { Button, ButtonClickEvent, TextArea, Form, Input, ImageUploader, Select } from "@meroedu/components"
import { AdminPageContainer } from "../components/AdminBasePage"
import { actions, Failure, Meroedu } from "@meroedu/services"
...
const GeneralSettingsPage = () => {
  ...
  const [title, setTitle] = useState<string>(meroedu.session.tenant.name)
  const [welcomeMessage, setWelcomeMessage] = useState<string>(meroedu.session.tenant.welcomeMessage)
  const [invitation, setInvitation] = useState<string>(meroedu.session.tenant.invitation)
  const [logo, setLogo] = useState<ImageUpload | undefined>(undefined)
  const [cname, setCNAME] = useState<string>(meroedu.session.tenant.cname)
  const [locale, setLocale] = useState<string>(meroedu.session.tenant.locale)
  ...
  const dnsInstructions = (): JSX.Element => {
    ...
    return (
      <>
        ...
        <strong>
          {meroedu.session.tenant.subdomain}
          {meroedu.settings.domain}
        </strong>
      </>
    )
  }
  ...
  return (
    <AdminPageContainer id="p-admin-general" name="general" title="General" subtitle="Manage your site settings">
      <Form error={error}>
        <Input field="title" label="Title" maxLength={60} value={title} disabled={!meroedu.session.user.isAdministrator} onChange={setTitle}>
        ...
        <TextArea
          ...
          disabled={!meroedu.session.user.isAdministrator}
        ...
        <ImageUploader label="Logo" field="logo" bkey={meroedu.session.tenant.logoBlobKey} disabled={!meroedu.session.user.isAdministrator} onChange={setLogo}>
          <p className="text-muted">
          ...
          {locale !== "en" && (
            <>
              <p className="text-muted">
                ...
                <a className="text-link" target="_blank" rel="noopener" href="https://crowdin.com/project/meroedu">
                  Crowdin
                </a>{" "}
        ...
        <div className="field">
          <Button disabled={!meroedu.session.user.isAdministrator} variant="primary" onClick={handleSave}>
            Save
          </Button>
        </div>
      </Form>
