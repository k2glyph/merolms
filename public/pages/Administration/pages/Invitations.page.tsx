...
import { Button, TextArea, Form, Input, Field } from "@meroedu/components"
import { actions, notify, Failure, Meroedu } from "@meroedu/services"
...
Regards,
${Meroedu.session.user.name} (${Meroedu.session.tenant.name})`,
      recipients: [],
      numOfRecipients: 0,
      rawRecipients: "",
    }
  ...
  private sendSample = async () => {
    ...
    if (result.ok) {
      notify.success(
        <span>
          An email message was sent to <strong>{Meroedu.session.user.email}</strong>
        </span>
      )
    }
    ...
    return (
      <Form error={this.state.error}>
        ...
        <Field label="Sample Invite">
          {Meroedu.session.user.email ? (
            <Button onClick={this.sendSample}>Send a sample email to {Meroedu.session.user.email}</Button>
          ) : (
