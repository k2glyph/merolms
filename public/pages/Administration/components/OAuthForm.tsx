...
import { OAuthConfig, OAuthConfigStatus, ImageUpload } from "@meroedu/models"
import { Failure, actions } from "@meroedu/services"
import { Form, Button, Input, SocialSignInButton, Field, ImageUploader, Toggle } from "@meroedu/components"
  ...
  return (
    <>
      ...
      <Form error={error}>
        ...
        <ImageUploader label="Logo" field="logo" bkey={logoBlobKey} disabled={!meroedu.session.user.isAdministrator} onChange={handleLogoChange}>
        ...
        <Input field="clientID" label="Client ID" maxLength={100} value={clientID} disabled={!meroedu.session.user.isAdministrator} onChange={setClientID} />
        ...
        />
        <Input field="tokenURL" label="Token URL" maxLength={300} value={tokenURL} disabled={!meroedu.session.user.isAdministrator} onChange={setTokenURL} />

        <Input field="scope" label="Scope" maxLength={100} value={scope} disabled={!meroedu.session.user.isAdministrator} onChange={setScope}>
          <p className="text-muted">
        ...
        <p className="text-muted">This section is used to configure how Meroedu will fetch user after the authentication process.</p>
        ...
        >
          <p className="text-muted">The URL to fetch the authenticated user info. If empty, Meroedu will try to parse the user info from the Access Token.</p>
        </Input>
