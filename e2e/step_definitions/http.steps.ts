...
import { MeroeduWorld } from "e2e/world"
...
const getFullUrl = (world: MeroeduWorld, url: string): string => {
  return url.startsWith("/") ? `https://${world.tenantName}.dev.meroedu.io:3000${url}` : url
}

Given("I prepare a {string} request to {string}", async function (this: MeroeduWorld, method: string, url: string) {
...
Given("I send a {string} request to {string}", async function (this: MeroeduWorld, method: string, url: string) {
